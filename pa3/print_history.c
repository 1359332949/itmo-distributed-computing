...
void print_history(const AllHistory * history)
{
	...
	printf("cccccccccccccccccccccccccccc %d === %d\n",history->s_history_len, history->s_history[5].s_history_len);
    for (int i = 1; i < history->s_history_len; ++i) {
        for (int j = 0; j <history->s_history[i].s_history_len; ++j) {
			...
			printf("i=%d, j=%d ppppp id=%d change->s_time=%d ppppp %d\n", i,j,id,change->s_time, change->s_balance);
    ...
    for (int j = 0; j <= max_time; ++j) {
        int sum = 0;
        for (int i = 1; i <= history->s_history_len; ++i) {
            sum += table[i][j].balance;
			//printf("%d sum:-----------%d\n", history->s_history_len, table[i][j].balance);
        }
    ...
    for (int i = 1; i <= history->s_history_len-1; ++i) {
        for (int j = 0; j <= max_time; ++j) {
            if (has_pending) {
